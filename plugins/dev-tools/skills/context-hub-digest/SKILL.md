...
### Step 2.5：拉全每个 plugin 的 WHY 材料（硬步骤，主线来源）

MR 正文与作者写给使用者的定位说明，是重点层（WHY）的来源。**这一步不是可选辅助**——每个有改动的 plugin，都必须先把窗口内动过它的所有 MR 正文（①-③）连同作者定位说明（④）拉全，再喂给对应 subagent，否则 subagent 只能从 diff 硬凑、必然抓错重点。
...
- **搭便车 commit**：某 plugin 的改动若合在别的主题分支/MR 里（例：某脚手架改动搭在另一个 SSO 文档 MR 里合入），没有属于自己的 MR 正文 → **不要直接跳到纯 diff 分析**，先走下面 ④ 的作者定位说明；只有 ④ 也拿不到时才回退纯 diff，据 diff 实际改动老实描述，不硬套那个无关 MR 的初衷。
- 正文 bullet **不出现 MR 编号**
- MR 一览（编号/合入时间/作者/summary 要点）仅在用户明确要求时作为附录输出，不进群聊正文

**④ 读作者写给使用者的定位说明（新增 plugin / 新增 skill 必做）：**

WHY 的来源不止 MR 正文。作者写给使用者看的定位文字常比 MR 正文更直接，而搭便车改动**往往一个 MR 正文都没有**（实测某窗口 4 个有改动 plugin 里 3 个如此）。所以完整回退链是：

```
MR 正文 → plugin README / plugin.json description / SKILL.md frontmatter → 才是纯 diff
```

```bash
# plugin 整体定位（新增 plugin 必读；plugin.json 的 description 本次新增了什么短语）
git show "$HEAD":plugins/<name>/README.md
git diff "$BASE" "$HEAD" -- plugins/<name>/.claude-plugin/plugin.json

# 新增 skill：读它的 frontmatter description 与「边界 / 现状 / 尚未实现」类章节
git show "$HEAD":plugins/<name>/skills/<skill>/SKILL.md
```

这里出现的定位边界是**硬约束**，直接决定某条改动能否进周报：

- README 说某 skill「门槛明显高于其他几个，面向内部评测，**普通使用者可以忽略**」→ 它不占 bullet，无论脚本多少行
- plugin.json description 本次**新增的那个短语**，通常就是作者给这次改动的一句话官方定位，可直接作为标签/描述的骨架
- README 开头的「解决的问题是……」通常就是该 plugin 的主线，比你从能力清单里归纳的"管生命周期"有信息量
- 「边界 / 尚未实现」表划清了不做什么——不写成 bullet，但用来防止把"不做的事"描述成能力

### Step 3：并行派发 subagent 做 plugin 级分析

对"有改动"的 plugin，**同一条消息里并行发出多个 Agent 调用**（每个 plugin 一个 subagent）：

- 每个 subagent 拿到**自己那一个 plugin** 的两份输入：`--stat`+`diff`（事实层 WHAT），以及 Step 2.5 拉全的该 plugin **MR 正文 + 作者定位说明**（重点层 WHY）。**主线来自作者写下的初衷，diff 用来核实事实和补细节**——按 Step 2 的 WHAT/WHY 分层办
...
- 输出固定结构（**含文件覆盖核对，不可省略**）：
  ...
  文件覆盖核对
  - <改动文件路径>: 主题 N / 次级项（<砍掉的理由>）
  - ...（`--stat` 里的每个文件都要有一行，一个不漏）
  ```

- **文件覆盖核对是防漏主线的机械检查**：`--stat` 列出的每个文件都要归到某个主题，或明确判为次级项并写明砍掉的理由。**未被任何主题解释的文件就是漏掉的主线**。一个 MR 改 5+ 个文件时，正文一句话常压着多条独立主线（实测：某 MR 正文"统一事实源、补齐关系约束"一句，实际盖着「语义保真」和「组合覆盖设计 + 测试锚点」两条独立主线，横跨 5 个文件；只提炼出前者就漏了后者）
...
#### 🔴 红线 8：禁内部校验/安全/门禁与正确性保障机制当用户价值

dry-run 试跑、二次确认、清单预览、权限/时效校验、幂等补齐这类**内部门禁与安全机制**，对读者是"看不见的底层保障"，默认不占 bullet。**同样不占 bullet 的还有内部正确性保障**：数据口径还原（如盲态判分 un-blind）、异常样本不进分母、分母口径修正、精度对齐——读者只感知到"结果是对的"，感知不到机制本身。只有当它**直接改变了用户的操作步骤**（例：以前一步完成、现在必须先确认再执行）时才值得一提，且并进它所服务的功能主题，不单列。

自检法：问"这条讲的是这个功能**能干什么**，还是它**内部怎么自我保护 / 怎么保证自己算得对**？"是后者 → 删或并入。

- ❌ 内部机制：「落库前都要 dry-run 真跑验证」「批量操作加二次确认护栏」「逾期罚息改为幂等补齐」
- ❌ 正确性保障：「内置盲态还原规则，避免拿原值汇总出错的胜率」「异常判分不进分母」
- ✅ 用户价值：「新增三个运维 harness：建 agent、写告警 SOP、管巡检」（验证/确认机制不单写）
- ✅ 用户价值：「可按判成某值、数值低于阈值、人机判得不一样筛 case」（口径还原机制不单写）

#### 🔴 红线 9：禁用体量或技术含量给主线排序

行数最多的文件、最难实现的机制、"这个 plugin 里唯一不可替代的部分"——**都不构成读者价值**。这是最隐蔽的一类错判：它看起来像在认真分析，实际是拿工程视角替换了读者视角，且**同一份周报里会反复犯**（在不同 plugin 上各犯一次，自己不会察觉是同一个错）。

作者在 README / plugin.json description / SKILL.md frontmatter 里划下的定位边界，**优先于**你从 diff 体量得出的任何判断：

| 你可能的判断 | 你的依据 | 作者的实际定位 |
|---|---|---|
| 「这是本次最重要的能力」 | 869 行，本 plugin 最大单文件 | README：「面向内部评测，**普通使用者可以忽略**」 |
| 「这是唯一不可替代的部分」 | 技术含量最高、别处没有 | 是看不见的正确性保障，读者只感知到"结果是对的"（红线 8） |

自检法：问"我说这条重要，是因为**读者会用到**，还是因为**它做起来难 / 代码多 / 别处没有**？"是后者 → 删。

作者说「可忽略」的，就不占 bullet，无论它有多少行。
...
- [ ] **新增 plugin / 新增 skill 的作者定位说明已读**（README、plugin.json description 本次新增的短语、SKILL.md frontmatter 与「边界/尚未实现」章节）；搭便车、无独立 MR 正文的改动是走完这一步后才回退纯 diff，不是直接跳到 diff
- [ ] **主线（WHY）来自作者写下的初衷，事实（WHAT）以 diff 为准**；没把 diff 里最显眼的机械改动（如删枚举值）误当主线
- [ ] **每个 subagent 都交回了文件覆盖核对**，`--stat` 里的文件一个不漏地归到某主题或被判为次级项；未被解释的文件已回头确认是不是漏掉的主线
...
- [ ] 无 dry-run/二次确认/权限校验等内部门禁机制，或盲态还原/分母口径这类正确性保障，被当用户价值单列（红线 8）
- [ ] **没有哪条 bullet 是靠"代码多/实现难/别处没有"入选的**；作者标注「普通使用者可忽略 / 面向内部」的能力已不占 bullet（红线 9）
