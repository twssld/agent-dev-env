...
sdd 是 context-hub 里最核心的 plugin，**不设 bullet 数上限**，且必须至少有一条走"意义分析"路线。

**"意义分析"不是空泛总结**，是回答一个具体问题：**这个改动对使用 sdd 的人意味着什么工作方式的改变？** 必须仍然包含具体 skill 名 / 文件名 / 字段作为落点。光描述"新增了什么阶段 / 改成了几档"只是动作罗列；要讲清这个改动让使用者之前的哪种做法不再成立。
...
最容易出"AI 味"的地方。不要把一组标签凑成整齐的四字/六字短语——人写周报不会那样。按内容自然长度来，2-3 字和 6-8 字混杂才正常。

- ❌ AI 味：「验收编号贯通」「门禁模型重塑」「阶段职责收敛」「轻量路径接通」（整齐四字）
- ✅ 参差：「验收场景统一编号」「tasks 精简」「clarify 限定在 WHAT/WHY」「sketch 接通 implement」

**3. 意义型 bullet 的标签尤其警惕（sdd 专属风险）**

给 sdd 写"意义分析"型 bullet 时最容易出 AI 味——讲意义天然想用抽象词，一不小心就组出「实现后的兜底」「轻量路径打通」「验收分层松绑」这种对仗短语。标签必须**直接点名 skill/文件/字段，像口语**：

- ❌ AI 味：「实现后的兜底」「轻量路径打通」「验收分层松绑」（抽象名词 + 文学化动词）
- ✅ 自然：「implement 自带复审」「implement支持sketch」「验收档位新增 E2E」
...
- [ ] **sdd 一节至少有一条"意义分析"型 bullet**，且其维度标签不是抽象对仗短语
