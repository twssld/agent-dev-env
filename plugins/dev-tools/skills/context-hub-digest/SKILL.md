...
## 范围
- 仓库：`/Users/taowei/code/context-hub`，目标 `plugins/` 下所有子目录（从 `origin/master` 动态枚举，不硬编码）
- 分支约束：只统计已合入 `origin/master` 的 commits
- 时间窗口：`$ARGUMENTS` 里解析时间短语为 `SINCE`/`END`（`YYYY-MM-DD`），默认 "过去 7 天"（`SINCE=今天-7d, END=今天-1d`，例：今天 29 号 → 22 ~ 28）。**`END` 始终 ≤ 今天-1d**；日期用 `date` 动态算；标题/招呼里的时段词与窗口一致；解析不明先追问
...
**动态枚举 `origin/master` 上的所有 plugin**（不要硬编码名单，也**不要用 `ls plugins/`**，那会混入本地未追踪或未推送的目录）：

```bash
# 从 origin/master 的 tree 中列出 plugins/ 下所有直接子目录，确保每个 plugin 都在远端 master 上真实存在
git ls-tree -d --name-only origin/master plugins/ | sed 's|^plugins/||'
```

如果有本地目录不在这份列表里（例如本地试验性 plugin 还没推远端），**直接忽略**，不纳入本次周报。

按"范围"一节解析出的 `SINCE` / `END`，在 `origin/master` 上拉窗口内 commit 列表：
...
**数据源锁定**：本 skill 全程只看 `origin/master` 上的 commit 与 diff，**不看本地工作区**（本地可能有未提交改动、未推送的分支、未追踪目录，和 origin 状态不一致）。如果 subagent 需要看某文件全文来补充上下文，必须用 `git show origin/master:<path>` 或 `git show $HEAD:<path>`，**不要直接 `Read` 本地文件**。
...
群聊消息，不是技术文档。顶部固定两行，时段词按解析出的窗口自然替换，其他照抄：
...
**c. 自检法**：把周报给自己读一遍，如果读起来像技术公众号标题 → AI 味过重，重写。

- ❌ AI 味：「验收体系重构」「评审闭环修复」「门禁模型重塑」「轻量路径接通」
- ✅ 自然：「验收方式统一」「code-reviewer 脚本修复」「tasks 模板精简」「sketch 接通 implement」

**d. 意义型 bullet 的标签尤其警惕**：给 sdd 写"意义分析"型 bullet 时最容易出 AI 味——因为"讲意义"天然想用抽象词，一不小心就组出「实现后的兜底」「轻量路径打通」「验收分层松绑」这种三字/五字对仗短语。标签必须用**非抽象、非文学化**的表述：
...
- [ ] Plugin 列表是通过 `git ls-tree origin/master plugins/` **从 git tree 枚举**出来的（不是 `ls plugins/`），本地未追踪目录已被排除
- [ ] 所有代码查看都走 `origin/master`（git log / git diff / git show origin/master:<path>），**没有用 `Read` 直接读本地文件**（本地可能与 origin 不一致）
...
- [ ] 时间窗口按 `$ARGUMENTS` 解析（默认过去 7 天；`END` ≤ 今天-1d），标题与打招呼的时段词与 `SINCE~END` 一致
...
```
📣 Context Hub 本周插件改动速览（2026-04-15 ~ 2026-04-21）
